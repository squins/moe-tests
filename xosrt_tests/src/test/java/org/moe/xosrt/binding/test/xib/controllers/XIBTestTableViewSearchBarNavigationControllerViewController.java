...
public class XIBTestTableViewSearchBarNavigationControllerViewController extends UIViewController implements
    ...
    protected XIBTestTableViewSearchBarNavigationControllerViewController(Pointer peer) {
        super(peer);
    }

    @Selector("alloc")
    public static native XIBTestTableViewSearchBarNavigationControllerViewController alloc();

    @Selector("init")
    public native XIBTestTableViewSearchBarNavigationControllerViewController init();
    ...
    public void tableViewDidSelectRowAtIndexPath(UITableView tableView, NSIndexPath indexPath) {
        UIViewController nextController = (XIBTestTableViewSearchBarNavigationController2ViewController)
                XIBTestTableViewSearchBarNavigationController2ViewController.alloc()
                        .initWithNibNameBundle("XIBTestTableViewSearchBarNavigationController2ViewController", null);
        navigationController().pushViewControllerAnimated(nextController, true);
    ...
    public void searchBarTextDidChange(UISearchBar searchBar, String searchText) {
        ...
        if (searchText.length() > 0) {
            ...
            filtered = _tableData.filteredArrayUsingPredicate(predicate);
            /*NSMutableArray filtered = NSMutableArray.alloc().init();

            for (String element : _tableData) {
                if (NSString.stringWithString(element).rangeOfString("searchText").location()
